#!/bin/bash
#
# Start script for payment-reconciliation-consumer
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/payment-reconciliation-consumer/env
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec "${APP_DIR}/payment-reconciliation-consumer" $(for broker in "${BROKERS[@]}"; do echo -n "-broker-addr=${broker} "; done)
